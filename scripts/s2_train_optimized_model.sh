#!/bin/bash
#
# Stage 2: Fine-tuning a Retriever with tDRO optimized weights
# Dataset sampling config: Obtained from tDRO
#
# @Author  :   Ma (Ma787639046@outlook.com)
#
BASE_DIR=$(dirname "$PWD")      # Get folder path of `tdro`

WEIGHT_TYPE=${1:-"curr"}        # curr / top70
DRO_TRAIL_NAME=${2:-"s1_tdro"}
DOMAIN_CONFIG_PATH=$BASE_DIR/results/${DRO_TRAIL_NAME}/${WEIGHT_TYPE}_weights.json    # Path to Dataset config, here we use weights from tDRO
TRAIL_NAME=${0%.*}_${WEIGHT_TYPE}      # Use filename as model's output dir name
