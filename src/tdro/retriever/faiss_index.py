...
# -*- encoding: utf-8 -*-
'''
Copied from beir/retrieval/search/dense
...
class FaissIndex:
    ...
    def search(self, query_embeddings: np.ndarray, k: int, **kwargs) -> Tuple[np.ndarray, np.ndarray]:
        ...
        logger.info("Num of queries: %d\tSearch time (s): %.3f\tQPS: %.3f", num_queries, total_time, qps)
    ...
    def build(
        ...
        for start in range(0, len(passage_ids), buffer_size):
    ...
    def to_gpu(self):
        if faiss.get_num_gpus() == 1:
            ...
            cloner_options.useFloat16 = False   # More accurate Flat search, but consuming more memory
